 ...
 * @author gabri
 */
public class AlterarMaoObra extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AlterarMaoObra</title>");
            out.println("</head>");
            out.println("<body>");
            try {
                out.println("ok");
                int id_pedido = Integer.parseInt(request.getParameter("id"));
                double valor = Double.parseDouble(request.getParameter("maoObra"));
                double preco = Double.parseDouble(request.getParameter("preco1"));
                double moAntigo = Double.parseDouble(request.getParameter("moAntigo"));
                preco = preco - moAntigo;
                preco = preco + valor;

                Pedido p = new Pedido();
                PedidoDAO pDAO = new PedidoDAO();
                p.setValor(preco);
                pDAO.maoObra(id_pedido, valor);
